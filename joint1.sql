...
select last_name,salary from workers where salary!=25000 and salary!=35000 and salary!=70000;
-- or 
...
select w.last_name,s.section_id,s.section_name from workers w,sections s where w.section_id=s.section_id;

-- Display the workers name along with their manager name
select m.first_name as 'worker',w.first_name as 'Mamager' from workers w,workers m where w.worker_id=m.manager_id;
-- or
select w.first_name as 'worker',m.first_name as 'Mamager' from workers w left join workers m on w.manager_id=m.worker_id;
-- Determine the number of managers without listing them.Label the column number of managers.
select count(distinct(manager_id)) as 'number of manager' from workers;
...
insert into studentnew value(1,'Abhi'),(2,'Aakash'),(3,'Roshan'),(4,'Neha');
insert into marksnew value(1,95),(2,80),(3,74),(4,81);
insert into studentnew value(5,'Komal'),(6,'Pooja');
insert into marksnew value(5,null);

select * from studentnew;
select * from marksnew;

-- find student name and marks whose mark are grater than student id 2
select totalmarks from marksnew where studentid=2;
select name,totalmarks from studentnew,marksnew where studentnew.studentid=marksnew.studentid and totalmarks>80;

-- by using sub-quire (inner/nested query and enclosed within parenthesis() abd execute first then after main query will execute)
select name,totalmarks from studentnew,marksnew where studentnew.studentid=marksnew.studentid and totalmarks>(select totalmarks from marksnew where studentid=2);


-- subquery is written by using colum and operators(comparison,between,logical,like,list(in,not in),exists,not exists,all,any)
-- group by having can be used in sub-query or in main query
-- but order by and limit cannot used in sub-query so used in main qyery
